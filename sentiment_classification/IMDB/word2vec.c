...
real *syn0, *syn1, *syn1neg, *expTable, *syn_avg, *syn_avg_neg, *syn_avg_haf, *syn_id, *syn_id_neg;
clock_t start;

real lambda = 1;
int avg_vectors=0;
long long sentence_num;
int hs = 0, negative = 5;
...
int GetSentenceId(char * word){
  if((int)strlen(word)<=1)return -1;
  if(!(word[0]=='_'&&word[1]=='*'))return -1;
  int sentence_id;
  char sentence_id_str[10];
  strncpy(sentence_id_str,word+2,(int)strlen(word)-2);
  sentence_id_str[(int)strlen(word)-2]='\0';
  sentence_id= atoi(sentence_id_str);
  return sentence_id; 
}
...
void *TrainModelThread(void *id) {
  ...
  long long sentence_id;
  char sentence_id_str[MAX_STRING];
  while (1) {
    ...
    ReadWord(sentence_id_str,fi);
    sentence_id=GetSentenceId(sentence_id_str);
    //if(sentence_id==-1)printf("%s\n",sentence_id_str);
      while (1) {
    ...
    if (cbow) {  //train the cbow architecture
      ...
      if (cw) {
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) {
          ...
          for (c = 0; c < layer1_size; c++) neu1e[c] += (1-lambda) * g * syn1neg[c + l2];
          for (c = 0; c < layer1_size; c++) syn1neg[c + l2] += (1-lambda) * g * neu1[c];
        }
        // hidden -> in
        for (a = b; a < window * 1 + 1 - b; a++) if (a != window) {
          c = sentence_position - window + a;
...
if(sentence_id!=-1){
      ...
      for(a=0;a<sentence_length;a++){
        ...
        if (negative  > 0) for (d = 0; d < negative + 1; d++) {
          ...
          l2 = target * layer1_size;
          f = 0;
          for (c = 0; c < layer1_size; c++) f += syn_id[c + l1] * syn0[c + l2];
          if (f > MAX_EXP) g = (label - 1) * alpha;
          else if (f < -MAX_EXP) g = (label - 0) * alpha;
          else g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
          for (c = 0; c < layer1_size; c++) neu1e[c] +=lambda * g * syn0[c + l2];
          for (c = 0; c < layer1_size; c++) syn0[c + l2] +=lambda * g * syn_id[c + l1];
        }
        for (c = 0; c < layer1_size; c++) syn_id[c+l1] +=neu1e[c];
      }
}


      if(local_iter==1&&avg_vectors==1&&sentence_id!=-1){
        l1=sentence_id*layer1_size;
        for(a=0;a<sentence_length;a++){
          l2=sen[a]*layer1_size;        
          if (sample > 0) {
            real ran = (sqrt(vocab[sen[a]].cn / (sample * train_words)) + 1) * (sample * train_words) / vocab[sen[a]].cn;
            next_random = next_random * (unsigned long long)25214903917 + 11;
            if (ran < (next_random & 0xFFFF) / (real)65536) continue;
          }
          for(c=0;c<layer1_size;c++)syn_avg[l1+c]+=syn0[l2+c];
        }
        for(c=0;c<layer1_size;c++)syn_avg[l1+c]=syn_avg[l1+c]/sentence_length;
      }
      sentence_length = 0;
      continue;
    }
...
void TrainModel() {
  ...
  if (classes == 0) {
    ...
    if(avg_vectors>=1){
      FILE *fo_avg;
      fo_avg = fopen((char *)"avg_vectors.txt","wb");
      for(a = 0; a < sentence_num; a++){
        fprintf(fo_avg, "%ld ", a);
        for(b=0;b<layer1_size;b++){
          fprintf(fo_avg, "%lf ", syn_avg[a*layer1_size+b]);
        }
        fprintf(fo_avg, "\n");
      }
      fclose(fo_avg);
...
int main(int argc, char **argv) {
  ...
  if ((i = ArgPos((char *)"-avg-vectors", argc, argv)) > 0) avg_vectors = atoi(argv[i + 1]);
  if ((i = ArgPos((char *)"-sentence-num", argc, argv)) > 0) sentence_num = atoi(argv[i + 1]);
